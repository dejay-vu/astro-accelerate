...
namespace astroaccelerate {
	...
	*/
	...
	private:
		...
		pulscan_workspace()
			: d_time_series(nullptr), d_time_series_capacity(0),
			  d_fft_output(nullptr), d_fft_capacity(0),
			  d_real(nullptr), d_real_capacity(0),
			  d_imag(nullptr), d_imag_capacity(0),
			  d_power(nullptr), d_power_capacity(0),
			  d_decimated2(nullptr), d_decimated2_capacity(0),
			  d_decimated3(nullptr), d_decimated3_capacity(0),
			  d_decimated4(nullptr), d_decimated4_capacity(0),
			  d_candidates_h1(nullptr), d_candidates_h1_capacity(0),
			  d_candidates_h2(nullptr), d_candidates_h2_capacity(0),
			  d_candidates_h3(nullptr), d_candidates_h3_capacity(0),
			  d_candidates_h4(nullptr), d_candidates_h4_capacity(0),
			  h_candidate_buffer(nullptr), h_candidate_capacity(0),
			  fft_plan(0), fft_length(0), fft_batch(0), fft_plan_valid(false),
			  h_pinned_batch(nullptr), h_pinned_capacity(0) {}
			...
			bool ensure_device(size_t elements) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&d_time_series), elements * sizeof(float));
				if (err != cudaSuccess) {
					return false;
				}
			...
			bool ensure_fft_output(size_t elements) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&d_fft_output), elements * sizeof(float2));
				if (err != cudaSuccess) {
					return false;
				}
			...
			bool ensure_fft_plan(int length, int batch) {
				if (fft_plan_valid && static_cast<size_t>(length) == fft_length && batch == fft_batch) {
					return true;
				}
				if (fft_plan_valid) {
			...
			bool ensure_real(size_t elements) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&d_real), elements * sizeof(float));
				if (err != cudaSuccess) {
					return false;
				}
			...
			bool ensure_imag(size_t elements) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&d_imag), elements * sizeof(float));
				if (err != cudaSuccess) {
					return false;
				}
			...
			bool ensure_power(size_t elements) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&d_power), elements * sizeof(float));
				if (err != cudaSuccess) {
					return false;
				}
			...
			bool ensure_decimated_buffer(float *&buffer, size_t &capacity, size_t elements) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&buffer), elements * sizeof(float));
				if (err != cudaSuccess) {
					return false;
				}
				capacity = elements;
			...
			bool ensure_candidates(size_t elements) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&d_candidates_h1), elements * sizeof(pulscan_candidate));
				if (err != cudaSuccess) {
					return false;
				}
			...
			bool ensure_candidates_harmonic(pulscan_candidate *&buffer, size_t &capacity, size_t elements) {
				if (elements <= capacity) {
					return true;
				}
				if (buffer) {
				...
				cudaError_t err = cudaMalloc(reinterpret_cast<void **>(&buffer), elements * sizeof(pulscan_candidate));
				if (err != cudaSuccess) {
					return false;
				}
				capacity = elements;
			...
			bool ensure_host(size_t elements) {
				...
				cudaError_t err = cudaMallocHost(reinterpret_cast<void **>(&h_pinned_batch), elements * sizeof(float));
				if (err != cudaSuccess) {
					return false;
				}
			...
			bool ensure_host_candidates(size_t elements) {
				...
				h_candidate_buffer = static_cast<pulscan_candidate *>(malloc(elements * sizeof(pulscan_candidate)));
				if (!h_candidate_buffer) {
					return false;
				}
		...
		*/
		...
		void set_pipeline_flags(){
			...
			if(do_pulscan_search) do_copy_DDTR_data_to_host = true;
		}
		...
		*/
		...
		bool pulscan() {
			...
			batch_plan.reserve(static_cast<size_t>(std::max(0, m_periodicity_strategy.nRanges())));
			...
			for (int r = 0; r < n_periodicity_ranges; ++r) {
				aa_periodicity_range current_range = m_periodicity_strategy.get_periodicity_range(static_cast<size_t>(r));
				...
				for (size_t b = 0; b < batch_count; ++b) {
					...
					min_samples_per_series = std::min(min_samples_per_series, samples_for_series);
					max_samples_per_series = std::max(max_samples_per_series, samples_for_series);
				}
			}

			printf("Pulscan staging prepared %zu batches and %zu DM series.\n", batch_plan.size(), total_dm_series);
			if (!batch_plan.empty()) {
				printf("Pulscan series length range: [%zu, %zu] samples (bin-corrected).\n", min_samples_per_series, max_samples_per_series);
			}
			else {
			...
			for (const auto &plan_entry : batch_plan) {
				...
				const size_t dm_series = static_cast<size_t>(std::max(0, plan_entry.dm_count));
				...
				if (!m_pulscan_workspace.ensure_host(batch_elements) || !m_pulscan_workspace.ensure_device(batch_elements)) {
				...
				for (int dm_idx = 0; dm_idx < plan_entry.dm_count; ++dm_idx) {
					float *series_ptr = m_output_buffer[plan_entry.range_index][plan_entry.dm_offset + dm_idx];
					if (series_ptr == nullptr) {
						continue;
					}
					std::copy_n(series_ptr, host_stride, host_buffer + static_cast<size_t>(dm_idx) * host_stride);
				}

				cuda_status = cudaMemcpy(m_pulscan_workspace.d_time_series, host_buffer, batch_elements * sizeof(float), cudaMemcpyHostToDevice);
				if (cuda_status != cudaSuccess) {
					LOG(log_level::error, "Pulscan: failed to copy time series batch to device (" + std::string(cudaGetErrorString(cuda_status)) + ")");
					pipeline_error = PIPELINE_ERROR_COPY_TO_DEVICE;
					break;
				}

				printf("Pulscan staged range %d batch %d: %d DM trials, %zu samples each.\n", plan_entry.range_index, plan_entry.batch_index, plan_entry.dm_count, host_stride);
				...
				if (!m_pulscan_workspace.ensure_fft_plan(fft_length, fft_batch)) {
					LOG(log_level::error, "Pulscan: failed to create cuFFT plan (length=" + std::to_string(fft_length) + ", batch=" + std::to_string(fft_batch) + ")");
					pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
					break;
				}
				...
				if (!m_pulscan_workspace.ensure_fft_output(fft_elements)) {
					LOG(log_level::error, "Pulscan: failed to allocate FFT output buffer for " + std::to_string(fft_elements) + " complex samples.");
					pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
					break;
				}

				cufftResult fft_status = cufftExecR2C(m_pulscan_workspace.fft_plan,
				...
				if (fft_status != CUFFT_SUCCESS) {
					LOG(log_level::error, "Pulscan: cuFFT execution failed (status=" + std::to_string(fft_status) + ")");
					pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
					break;
				}

				printf("Pulscan FFT completed for range %d batch %d (%d series, %zu frequency bins).\n",
				...
				if (!m_pulscan_workspace.ensure_real(freq_bins) ||
					...
					LOG(log_level::error, "Pulscan: failed to allocate magnitude workspace of size " + std::to_string(freq_bins));
					pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
					break;
				}
				...
				for (int dm_idx = 0; dm_idx < plan_entry.dm_count; ++dm_idx) {
					...
					float2 *dm_fft = m_pulscan_workspace.d_fft_output + static_cast<size_t>(dm_idx) * freq_bins;
					const dim3 fftBlock(boxcarThreadCount);
					const dim3 fftGrid((static_cast<int>(freq_bins) + boxcarThreadCount - 1) / boxcarThreadCount);

					call_kernel_pulscan_separate_components(fftGrid, fftBlock, stream,
						dm_fft,
						m_pulscan_workspace.d_real,
						m_pulscan_workspace.d_imag,
						static_cast<long>(freq_bins));

					const long normaliseElements = static_cast<long>(freq_bins);
					const long normaliseBlockElements = 4096;
					if (normaliseElements >= normaliseBlockElements && (normaliseElements % normaliseBlockElements) == 0) {
						const dim3 normaliseGrid(static_cast<unsigned int>(normaliseElements / normaliseBlockElements));
						const dim3 normaliseBlock(1024);
						call_kernel_pulscan_median_normalisation(normaliseGrid, normaliseBlock, stream, m_pulscan_workspace.d_real);
						call_kernel_pulscan_median_normalisation(normaliseGrid, normaliseBlock, stream, m_pulscan_workspace.d_imag);
					} else if (!pulscan_warned_normalisation) {
						LOG(log_level::warning, "Pulscan: skipping median normalisation because frequency bin count " + std::to_string(freq_bins) + " is not a multiple of 4096.");
						pulscan_warned_normalisation = true;
					}

					call_kernel_pulscan_magnitude_squared(fftGrid, fftBlock, stream,
						m_pulscan_workspace.d_real,
						m_pulscan_workspace.d_imag,
					...
					if (cuda_status != cudaSuccess) {
						LOG(log_level::error, "Pulscan: magnitude kernels failed (" + std::string(cudaGetErrorString(cuda_status)) + ")");
						pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
						break;
					}
					...
					if ((dec2_bins > 0 && !m_pulscan_workspace.ensure_decimated_buffer(m_pulscan_workspace.d_decimated2, m_pulscan_workspace.d_decimated2_capacity, dec2_bins)) ||
						(dec3_bins > 0 && !m_pulscan_workspace.ensure_decimated_buffer(m_pulscan_workspace.d_decimated3, m_pulscan_workspace.d_decimated3_capacity, dec3_bins)) ||
						(dec4_bins > 0 && !m_pulscan_workspace.ensure_decimated_buffer(m_pulscan_workspace.d_decimated4, m_pulscan_workspace.d_decimated4_capacity, dec4_bins))) {
						LOG(log_level::error, "Pulscan: failed to allocate decimated power buffers.");
						pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
						break;
					}

					const bool run_decimation = (dec2_bins > 0 || dec3_bins > 0 || dec4_bins > 0);
					if (run_decimation) {
						const int decimationThreadCount = boxcarThreadCount;
						const int decimationGrid = static_cast<int>((static_cast<long>(dec2_bins) + decimationThreadCount - 1) / decimationThreadCount);
						if (decimationGrid > 0) {
							...
							cuda_status = cudaGetLastError();
							if (cuda_status != cudaSuccess) {
								LOG(log_level::error, "Pulscan: decimation kernel failed (" + std::string(cudaGetErrorString(cuda_status)) + ")");
								pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
								break;
							}
						}
					...
					auto try_register_harmonic = [&](int num_harmonics, float *input, size_t length, pulscan_candidate *&buffer, size_t &capacity) {
						...
						int gridSize = static_cast<int>((validThreads + boxcarThreadCount - 1) / boxcarThreadCount);
						if (gridSize <= 0) {
							return;
						}
						size_t candidateCount = static_cast<size_t>(gridSize) * static_cast<size_t>(numCandidatesPerBlock);
						if (candidateCount == 0) {
							return;
						}
						bool ensured = (num_harmonics == 1)
							? m_pulscan_workspace.ensure_candidates(candidateCount)
							: m_pulscan_workspace.ensure_candidates_harmonic(buffer, capacity, candidateCount);
						if (!ensured) {
							LOG(log_level::error, "Pulscan: failed to allocate candidate buffer for harmonic " + std::to_string(num_harmonics));
							pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
							return;
						}

						cuda_status = cudaMemset(buffer, 0, candidateCount * sizeof(pulscan_candidate));
						if (cuda_status != cudaSuccess) {
							LOG(log_level::error, "Pulscan: failed to zero candidate buffer (" + std::string(cudaGetErrorString(cuda_status)) + ")");
							pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
							return;
						}
					...
					try_register_harmonic(1, m_pulscan_workspace.d_power, freq_bins, m_pulscan_workspace.d_candidates_h1, m_pulscan_workspace.d_candidates_h1_capacity);
					try_register_harmonic(2, m_pulscan_workspace.d_decimated2, dec2_bins, m_pulscan_workspace.d_candidates_h2, m_pulscan_workspace.d_candidates_h2_capacity);
					try_register_harmonic(3, m_pulscan_workspace.d_decimated3, dec3_bins, m_pulscan_workspace.d_candidates_h3, m_pulscan_workspace.d_candidates_h3_capacity);
					try_register_harmonic(4, m_pulscan_workspace.d_decimated4, dec4_bins, m_pulscan_workspace.d_candidates_h4, m_pulscan_workspace.d_candidates_h4_capacity);
					...
					for (size_t h = 0; h < harmonic_run_count; ++h) {
						...
						call_kernel_pulscan_boxcar_filter(
							dim3(run.grid_size),
					...
					for (size_t h = 0; h < harmonic_run_count; ++h) {
						harmonic_run &run = harmonic_runs[h];
						const int logpGrid = static_cast<int>((static_cast<long>(run.candidate_count) + boxcarThreadCount - 1) / boxcarThreadCount);
					...
					if (cuda_status != cudaSuccess) {
						LOG(log_level::error, "Pulscan: candidate kernels failed (" + std::string(cudaGetErrorString(cuda_status)) + ")");
						pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
						break;
					}
					...
					if (!m_pulscan_workspace.ensure_host_candidates(total_candidates)) {
						LOG(log_level::error, "Pulscan: failed to allocate host candidate buffer for " + std::to_string(total_candidates) + " entries.");
						pipeline_error = PIPELINE_ERROR_GENERAL_GPU_ERROR;
						break;
					}
					...
					for (size_t h = 0; h < harmonic_run_count; ++h) {
						...
						if (run.candidate_count > 0) {
							cuda_status = cudaMemcpy(m_pulscan_workspace.h_candidate_buffer + host_offset,
								run.device_candidates,
								run.candidate_count * sizeof(pulscan_candidate),
								cudaMemcpyDeviceToHost);
							if (cuda_status != cudaSuccess) {
								LOG(log_level::error, "Pulscan: failed to copy candidates to host (" + std::string(cudaGetErrorString(cuda_status)) + ")");
								pipeline_error = PIPELINE_ERROR_COPY_TO_HOST;
								break;
							}
						}
					...
					double dm_value = plan_entry.dm_low + static_cast<double>(plan_entry.dm_offset + dm_idx) * plan_entry.dm_step;
					size_t candidates_added = 0;
					double dt = plan_entry.sampling_time * static_cast<double>(plan_entry.in_bin);
					...
					for (size_t h = 0; h < harmonic_run_count; ++h) {
						harmonic_run &run = harmonic_runs[h];
						pulscan_candidate *host_slice = m_pulscan_workspace.h_candidate_buffer + run.host_offset;
						for (size_t c = 0; c < run.candidate_count; ++c) {
							pulscan_candidate candidate = host_slice[c];
							if (candidate.logp >= logpThreshold || candidate.z == 0 || candidate.r == 0) {
								continue;
							}
							if (candidate.r >= static_cast<int>(freq_bins)) {
								continue;
							}
							double frequency_hz = static_cast<double>(candidate.r) / denom;
			...
			std::sort(m_pulscan_candidates.begin(), m_pulscan_candidates.end(), [](const pulscan_host_candidate &a, const pulscan_host_candidate &b) {
				return a.sigma > b.sigma;
			});
			const size_t max_pulscan_candidates = 4096;
			if (m_pulscan_candidates.size() > max_pulscan_candidates) {
				m_pulscan_candidates.resize(max_pulscan_candidates);
			}
...
#if AA_WITH_PULSCAN
		const std::vector<pulscan_host_candidate> &get_pulscan_candidates() const override {
