...
/**
 * \class aa_permitted_pipelines_5 aa_permitted_pipelines_5.hpp
 * "include/aa_permitted_pipelines_5.hpp" \brief Templated class to run
 * dedispersion and analysis and periodicity and acceleration. \details The
 * class is templated over the zero_dm_type
 * (aa_pipeline::component_option::zero_dm or
 * aa_pipeline::component_option::zero_dm_with_outliers). \author
 * AstroAccelerate team.
 */
...
private:
  ...
  const aa_pipeline::pipeline
      m_pipeline_components; /** The user requested pipeline that was bound to
                                the aa_pipeline_api instance on construction. */
  const aa_pipeline::pipeline_option
      m_pipeline_options; /** The user requested pipeline details containing
                             component options for the aa_pipeline_api instance.
                           */
  ...
  bool cleanup_DDTR() {
    ...
    if(d_DDTR_input != NULL) {
      ...
      if(e != cudaSuccess) {
        ...
        LOG(log_level::error,
            "Cannot free memory (" + std::string(cudaGetErrorString(e)) + ")");
      }
      else {
        d_DDTR_input = NULL;
      }
    ...
    if(e != cudaSuccess) {
      pipeline_error = PIPELINE_ERROR_GPU_FREE_MEMORY_FAIL;
      LOG(log_level::error,
          "Cannot free d_bandpass_normalization memory: (" +
  ...
  bool allocate_gpu_memory_DDTR() {
    // const int &maxshift, const int &max_ndms, const int &nchans, int **const
    // t_processed, unsigned short **const d_DDTR_input, float **const
    // d_DDTR_output, float **const d_dm_shifts) {
    ...
    printf("DDTR input size: %zu bytes = %0.3f MB;\n",
           gpu_inputsize,
           ((double)gpu_inputsize) / (1024.0 * 1024.0));
    printf("DDTR output size:  %zu bytes = %0.3f MB;\n",
           gpu_outputsize,
           ((double)gpu_outputsize) / (1024.0 * 1024.0));
    if(nchans > 8192) {
      printf("Channel shifts: %zu bytes = %0.3f MB;\n",
             nchans * sizeof(float),
             ((double)nchans * sizeof(float)) / (1024.0 * 1024.0));
    ...
    printf("Total memory required: %zu bytes = %0.3f MB;\n",
           required_memory,
           ((double)required_memory) / (1024.0 * 1024.0));
    ...
    if(e != cudaSuccess) {
      ...
      LOG(log_level::error,
          "Could not allocate memory for d_DDTR_input using cudaMalloc in "
          "aa_permitted_pipelines_generic.hpp (" +
              std::string(cudaGetErrorString(e)) + ")");
    }
    ...
    if(e != cudaSuccess) {
      pipeline_error = PIPELINE_ERROR_DDTR_GPU_MEMORY_FAIL;
      LOG(log_level::error,
          "Could not allocate memory for d_DDTR_output using cudaMalloc in "
          "aa_permitted_pipelines_generic.hpp (" +
              std::string(cudaGetErrorString(e)) + ")");
    }
    ...
    if((nchans > 8192) && (nbits != 4)) {
      ...
      if(e != cudaSuccess) {
        pipeline_error = PIPELINE_ERROR_DDTR_GPU_MEMORY_FAIL;
        LOG(log_level::error,
            "Could not allocate memory for d_dm_shifts using cudaMalloc in "
            "aa_permitted_pipelines_generic.hpp (" +
                std::string(cudaGetErrorString(e)) + ")");
      }
    }
    ...
    if((nchans > 4096) && (nbits == 4)) {
      ...
      if(e != cudaSuccess) {
        pipeline_error = PIPELINE_ERROR_DDTR_GPU_MEMORY_FAIL;
        LOG(log_level::error,
            "Could not allocate memory for d_dm_shifts (4-bit) using "
            "cudaMalloc in aa_permitted_pipelines_generic.hpp (" +
                std::string(cudaGetErrorString(e)) + ")");
      }
    }
    ...
    if(e != cudaSuccess) {
      pipeline_error = PIPELINE_ERROR_DDTR_GPU_MEMORY_FAIL;
      LOG(log_level::error,
          "Could not allocate memory for d_bandpass_normalization using "
          "cudaMalloc in aa_permitted_pipelines_generic.hpp (" +
              std::string(cudaGetErrorString(e)) + ")");
    }
  ...
  void allocate_cpu_memory_SPD(size_t max_nDMs, size_t timesamples_per_chunk) {
    ...
    h_SPD_candidate_list_DM =
        (unsigned int*)malloc(SPD_max_peak_size * sizeof(unsigned int));
    h_SPD_candidate_list_TS =
        (unsigned int*)malloc(SPD_max_peak_size * sizeof(unsigned int));
    h_SPD_candidate_list_SNR =
        (float*)malloc(SPD_max_peak_size * sizeof(float));
    h_SPD_candidate_list_BW =
        (unsigned int*)malloc(SPD_max_peak_size * sizeof(unsigned int));
    if(h_SPD_candidate_list_DM == NULL || h_SPD_candidate_list_TS == NULL ||
       h_SPD_candidate_list_SNR == NULL || h_SPD_candidate_list_BW == NULL) {
      pipeline_error = PIPELINE_ERROR_HOST_MEMORY_FAIL;
      LOG(log_level::error,
          "Could not allocate memory on the host for single pulse detection "
          "candidates");
    }
  }
   ...
   */
  void allocate_gpu_memory_SPD(float** const            d_MSD_workarea,
                               unsigned short** const   d_SPDT_output_taps,
                               float** const            d_MSD_interpolated,
                               const unsigned long int& MSD_maxtimesamples,
                               const size_t&            MSD_profile_size) {
    cudaError_t e;

    e = cudaMalloc((void**)d_MSD_workarea,
                   MSD_maxtimesamples * 5.5 * sizeof(float));
    if(e != cudaSuccess) {
      pipeline_error = PIPELINE_ERROR_SPDT_GPU_MEMORY_FAIL;
      LOG(log_level::error,
          "Could not allocate memory for d_MSD_workarea using cudaMalloc in "
          "aa_permitted_pipelines_generic.hpp (" +
              std::string(cudaGetErrorString(e)) + ")");
    }

    e = cudaMalloc((void**)&(*d_SPDT_output_taps),
                   sizeof(ushort) * 2 * MSD_maxtimesamples);
    if(e != cudaSuccess) {
      pipeline_error = PIPELINE_ERROR_SPDT_GPU_MEMORY_FAIL;
      LOG(log_level::error,
          "Could not allocate memory for d_SPDT_output_taps using cudaMalloc "
          "in aa_permitted_pipelines_generic.hpp (" +
              std::string(cudaGetErrorString(e)) + ")");
    }

    e = cudaMalloc((void**)d_MSD_interpolated,
                   sizeof(float) * MSD_profile_size);
    if(e != cudaSuccess) {
      pipeline_error = PIPELINE_ERROR_SPDT_GPU_MEMORY_FAIL;
      LOG(log_level::error,
          "Could not allocate memory for d_MSD_interpolated cudaMalloc in "
          "aa_permitted_pipelines_generic.hpp (" +
              std::string(cudaGetErrorString(e)) + ")");
    }
  }
  ...
  /**
   * \brief Allocate a 3D array that is an output buffer that stores dedispersed
   * array data. \details This array is used by periodicity.
   */
  ...
  bool set_data() {
    ...
    // allocate_gpu_memory_DDTR(maxshift, max_ndms, nchans, t_processed,
    // &d_DDTR_input, &d_DDTR_output, &d_dm_shifts);
    allocate_gpu_memory_DDTR();

    if(m_ddtr_strategy.bandpass_normalization_size() == (size_t)nchans) {
      cudaMemcpy(d_bandpass_normalization,
                 m_ddtr_strategy.bandpass_normalization_pointer(),
                 nchans * sizeof(float),
    ...
    if(do_single_pulse_detection && pipeline_error == 0) {
      ...
      allocate_gpu_memory_SPD(&m_d_MSD_workarea,
                              &m_d_SPDT_output_taps,
                              &m_d_MSD_interpolated,
                              m_analysis_strategy.MSD_data_info(),
                              m_analysis_strategy.MSD_profile_size_in_bytes());
    }
    ...
    // This is needed so that the kernel wrapper functions don't need to be
    // modified.
  ...
  bool input_data_preprocessing() {
    ...
    const aa_pipeline::component_option opt_set_bandpass_average =
        aa_pipeline::component_option::set_bandpass_average;
    if(m_pipeline_options.find(opt_set_bandpass_average) !=
       m_pipeline_options.end()) {
      ...
      cuda_error = cudaMemcpy(d_bandpass_normalization,
                              h_new_bandpass,
                              nchans * sizeof(float),
                              cudaMemcpyHostToDevice);
  ...
  /** \brief Method that pipeline flags according to requested pipeline
   * components. */
  void set_pipeline_flags() {
    //----> Components
    // const aa_pipeline::component cmp_dedispersion =
    // aa_pipeline::component::dedispersion;
    const aa_pipeline::component cmp_analysis =
        aa_pipeline::component::analysis;
    const aa_pipeline::component cmp_periodicity =
        aa_pipeline::component::periodicity;
    ...
    //----> Component options
    const aa_pipeline::component_option opt_copy_ddtr_data_to_host =
        aa_pipeline::component_option::copy_ddtr_data_to_host;

    do_dedispersion = true; // default component
    if(m_pipeline_components.find(cmp_analysis) != m_pipeline_components.end())
      do_single_pulse_detection = true;
    else
      do_single_pulse_detection = false;
    if(m_pipeline_components.find(cmp_periodicity) !=
       m_pipeline_components.end())
      do_periodicity_search = true;
    else
      do_periodicity_search = false;
    if(m_pipeline_components.find(cmp_fdas) != m_pipeline_components.end())
      do_fdas = true;
    else
      do_fdas = false;
    if(m_pipeline_components.find(cmp_jerk) != m_pipeline_components.end())
      do_jerk = true;
    else
      do_jerk = false;
  ...
  do_pulscan_search = do_periodicity_search;
    ...
    if(m_pipeline_options.find(opt_copy_ddtr_data_to_host) !=
       m_pipeline_options.end())
      do_copy_DDTR_data_to_host = true;
    if(do_periodicity_search)
      do_copy_DDTR_data_to_host = true;
    if(do_fdas)
      do_copy_DDTR_data_to_host = true;
    if(do_jerk)
      do_copy_DDTR_data_to_host = true;
    if(do_pulscan_search)
      do_copy_DDTR_data_to_host = true;
  }

  /**
   * \brief Run the pipeline by processing the next time chunk of data.
   * \details Process any flags for dumping output or providing it back to the
   * user. \details aa_permitted_pipelines_5 does not return intermediate data.
   * \details If this is required, then optional parameters can be passed as
   * arguments which would contain periodicity and acceleration output. \details
   * Since it is the pipelines perogative to decide when periodicity and
   * acceleration are run, the user is responsible for checking when these
   * objects were actually modified by the pipeline. \details The user is
   * expected to keep calling run_pipeline until it returns false. \details
   * Returning true indicates there is more to process. \details Returning false
   * indicates the pipeline is finished running. \returns A boolean to indicate
   * whether further time chunks are available to process (true) or not (false).
   */
  bool run_pipeline(aa_pipeline_runner::status& status_code) {
    const aa_pipeline::component_option opt_zero_dm =
        aa_pipeline::component_option::zero_dm;
    const aa_pipeline::component_option opt_zero_dm_with_outliers =
        aa_pipeline::component_option::zero_dm_with_outliers;
    const aa_pipeline::component_option opt_input_DDTR_normalization =
    ...
    // aa_pipeline::component_option::dered;
    ...
    if(CUDA_error != cudaSuccess) {
      ...
      LOG(log_level::error,
          "GPU error at the pipeline start. (" +
    ...
    if(current_time_chunk >= num_tchunks) {
      ...
      if(!did_notify_of_finishing_component) {
        ...
        printf("\n\n === OVERALL DEDISPERSION THROUGHPUT INCLUDING SYNCS AND "
               "DATA TRANSFERS ===\n");
        printf("\n(Performed Brute-Force Dedispersion: %g (GPU estimate)",
               time);
      ...
      return false; // In this case, there are no more chunks to process, and
                    // periodicity and acceleration both ran.
    }
    ...
    if(current_range == 0 && pipeline_error == PIPELINE_ERROR_NO_ERROR) {
      ...
      printf("\nNOTICE: t_processed:\t%d, %d",
             t_processed[0][current_time_chunk],
             current_time_chunk);

      //---------> Load chunks
      m_local_timer.Start();
      load_chunk_data(d_DDTR_input,
                      &m_input_buffer[(long int)(inc * nchans)],
                      t_processed[0][current_time_chunk],
                      maxshift_original,
                      nchans,
                      dmshifts,
      ...
      if(m_pipeline_options.find(opt_zero_dm) != m_pipeline_options.end()) {
        ...
        m_local_timer.Start();
        zero_dm(d_DDTR_input,
                nchans,
                t_processed[0][current_time_chunk] + maxshift_original,
                nbits,
                d_bandpass_normalization);
        m_local_timer.Stop();
        time_log.adding("DDTR", "Zero_DM", m_local_timer.Elapsed());
      }
      else if(m_pipeline_options.find(opt_zero_dm_with_outliers) !=
              m_pipeline_options.end()) {
        LOG(log_level::debug, "Performing zero DM with outliers...");
        m_local_timer.Start();
        zero_dm_outliers(d_DDTR_input,
                         nchans,
                         t_processed[0][current_time_chunk] + maxshift_original,
                         nbits,
                         d_bandpass_normalization);
      ...
      if(m_pipeline_options.find(opt_input_DDTR_normalization) !=
         m_pipeline_options.end()) {
      ...
      //---------> Corner turn
      m_local_timer.Start();
      corner_turn_SM_inplace(
          (unsigned short*)d_DDTR_input,
          (unsigned short*)d_DDTR_output,
          (size_t)nchans,
          (size_t)(t_processed[0][current_time_chunk] + maxshift_original));
      m_local_timer.Stop();
    ...
    if(current_time_chunk < num_tchunks &&
      ...
      printf("\n\nNOTICE: %f\t%f\t%f\t%d\n",
             m_ddtr_strategy.dm(dm_range).low,
             m_ddtr_strategy.dm(dm_range).high,
             m_ddtr_strategy.dm(dm_range).step,
             m_ddtr_strategy.ndms(dm_range));
      ...
      m_local_timer.Start();
      set_dedispersion_constants(t_processed[dm_range][current_time_chunk],
                                 maxshift);
      m_local_timer.Stop();
      time_log.adding("DDTR", "Host_To_Device", m_local_timer.Elapsed());
      ...
      if(CUDA_error != cudaSuccess) {
        ...
        LOG(log_level::error,
            "GPU error at Dedispersion. (" +
                std::string(cudaGetErrorString(CUDA_error)) + ")");
      }

      if(inBin[dm_range] > oldBin) {
        m_local_timer.Start();
        bin_gpu(d_DDTR_input,
                d_DDTR_output,
                nchans,
                t_processed[dm_range - 1][current_time_chunk] +
                    maxshift * inBin[dm_range]);
        m_local_timer.Stop();
      ...
      if(m_pipeline_options.find(opt_output_DDTR_normalization) !=
         m_pipeline_options.end()) {
      ...
      if(do_copy_DDTR_data_to_host) {
        m_local_timer.Start();
        save_data_offset_stream(dm_range,
                                current_time_chunk,
                                t_processed,
                                inc,
                                inBin.data(),
      ...
      if(do_single_pulse_detection &&
        ...
        SPD_nCandidates = 0;
        SPDT_no_error =
            analysis_GPU(h_SPD_candidate_list_DM,
                         h_SPD_candidate_list_TS,
                         h_SPD_candidate_list_SNR,
                         h_SPD_candidate_list_BW,
                         &SPD_nCandidates,
    ...
    printf("NOTICE: Pipeline ended run_pipeline_generic over chunk %d / %d and "
           "range %d / %d.\n",
           current_time_chunk,
           num_tchunks,
           current_range,
  ...
  bool periodicity() {
    ...
    //  printf("\n\n === OVERALL PERIODICITY THROUGHPUT INCLUDING SYNCS AND DATA
    //  TRANSFERS ===\n"); printf("\nPerformed Periodicity Location: %f (GPU
    //  estimate)", time); printf("\nAmount of telescope time processed: %f",
    //  tstart_local); printf("\nNumber of samples processed: %ld", inc);
  ...
  bool acceleration() {
    ...
    time_log.adding("Total", "total", timer.Elapsed());
    float time = timer.Elapsed() / 1000;
    printf("\n\n === OVERALL TDAS THROUGHPUT INCLUDING SYNCS AND DATA "
           "TRANSFERS ===\n");
  ...
  bool jerk_search() {
    ...
    jerk_search_from_ddtr_plan(m_output_buffer,
                               m_jerk_strategy,
                               dm_low.data(),
                               dm_step.data(),
                               ndms,
...
public:
  ...
  aa_permitted_pipelines_generic(const aa_permitted_pipelines_generic&) =
      delete;
  ...
  /** \brief Override base class next() method to process next time chunk. Also
   ...
   * \details The array data is only useful once the pipeline has finished
   * running. \details Users should finish running the pipeline so that all
   * dedispersion output is available. \details Alternatively, the user may
   * access the data one time chunk at a time, but the next time chunk will not
   * have been computed yet. \details The structure of the ddtr output buffer
   * data is indexed by: time_chunk index, dm_range index, dm.
   */
  float*** output_buffer() {
    if(memory_allocated) {
  ...
  size_t get_SPD_nCandidates() { return SPD_nCandidates; }
  ...
  float* Get_PSR_interbin_candidates() {
    ...
    c_h_PSR_interbin_candidates =
        (float*)malloc(nCandidates * 4 * sizeof(float));
  ...
  size_t get_PSR_nCandidates() { return c_PSR_power_candidates.nCandidates(); }
  ...
  void Write_to_disk_Pulscan_candidates(const char* filename) override {
    if(!filename) {
      LOG(log_level::error, "Pulscan: invalid output filename (null pointer).");
      return;
    }
    ...
    if(!fp) {
      LOG(log_level::error,
          "Pulscan: failed to open output file " + std::string(filename));
      return;
    }
  ...
  bool cleanup() {
    if(memory_allocated && !memory_cleanup) {
      LOG(log_level::debug,
          "Generic Pipeline -> Memory cleanup at the end of the pipeline");
  ...
  m_pulscan_runner.release();
    ...
    if(m_pipeline_options.find(
           aa_pipeline::component_option::timelog_export_to_file) !=
       m_pipeline_options.end()) {
